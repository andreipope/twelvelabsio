...
![](/images/Twelve%20Labs%20-%20Architecture%20v3.png)
...
* **Engines**: An engine indexes your videos and allows you to search for particular information. Each engine uses its own family of deep learning models to index data and has a pricing model associated with it. For details about pricing, contact us at sales\[at\]twelvelabs.io.
...
* **Indexes**: An index groups one or more video vectors and is the most granular level at which you can perform a search. When you create an index, you must specify the following properties:  
   ...
   * Engine  
   * Indexing options  
Note that you cannot modify the engine or the indexing options once the index has been created. Indexes use a push model, meaning that you can upload videos whenever you like.
...
2. 2.  
Upload a video, specifying the unique identifier of the index with which the video will be associated. The deep learning models will process your video according to the indexing options you’ve specified for the index, and store it as a video vector. The API creates a video indexing task and assigns it a unique identifier.

3. 3.  
Perform a search by providing your search terms and the unique identifier of the index you want to query. For each match found, the search engine will return, among other information, the unique identifier of the video, and the start and end time of the matching video fragment.